...
template <typename value_type>
inline void Matrix<value_type>::print() const {
...
template <typename value_type>
inline std::shared_ptr<Matrix<double>> Matrix<value_type>::inverse_LU_method(double eps) const {
...
std::shared_ptr<Matrix<double>> Matrix<value_type>::inverse() const {
	std::shared_ptr<Matrix<double>> inverse;
	if (this->msize() == 0) {
		std::cout << "求逆错误：矩阵为空" << endl;
		return inverse;
	}
	if (this->msize() != this->nsize()) { 
		std::cout << "求逆错误：矩阵不是方阵" << endl;
		return inverse;
	}
...
double Matrix<value_type>::determinant() const {
	if (this->msize() == 0) {
		std::cout << "行列式错误：矩阵为空" << endl;
		return 0;
	}
	if (this->msize() != this->nsize()) {
		std::cout << "行列式错误：矩阵不是方阵" << endl;
		return 0;
	}
