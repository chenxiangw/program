...
void Gauss_Jordan(Matrix<double> &A, Matrix<double> &b) {
	...
	*/
	...
	for (int i = 0; i < m; ++i) {
		...
		//主元所在行归一化
		if(A[pivot_col][pivot_col]==0.0) throw "无逆矩阵";
...
void householder_transform(Matrix<double> &A, Matrix<double> &U, vector<double> &S, Matrix<double> &V, vector<double> &e, double eps = EPS) {
	/*
...
int SVD_decomposition(Matrix<double> &A, Matrix<double> &U, vector<double> &S, Matrix<double> &V, double eps = EPS){
	/*
		奇异值分解
	...
	*/
	...
	vector<double> e(n);
	//householder分解
	householder_transform(A, U, S, V, e, eps);
